...
const ProjectDetailModal: React.FC<ProjectDetailModalProps> = ({ 
  ...
  // 解析Markdown内容
  useEffect(() => {
    if (project.content || project.description) {
      try {
        ...
        setParsedContent(`<p>${project.content || project.description}</p>`);
      }
    }
    ...
    // 图片加载计数
    console.log(`总共 ${images.length} 张图片需要加载`);
    
    // 输出所有图片地址到控制台以便调试
    images.forEach((image, index) => {
      console.log(`图片 ${index + 1}: ${image}`);
    });
  }, [project.description, project.content, project.detailImages, project.images]);
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-8 md:gap-10 mb-8 md:mb-10" style={{ width: '100%' }}>
            {(project.detailImages || project.images || []).map((media, index) => {
