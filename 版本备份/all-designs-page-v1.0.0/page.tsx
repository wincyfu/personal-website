...
export default function AllDesignsPage() {
  ...
  const categories = [
    { 
      id: 'all', 
      name: isEnglish ? 'All Projects' : '全部项目' 
    },
    { 
      id: 'mobile', 
            ...
            <div key={project.id} className="group cursor-pointer" onClick={(e) => handleCardClick(e, project)}>
              ...
              >
                ...
                <div className="px-[20px] pt-[24px] pb-[20px] flex flex-col h-full">
                  <h3 className={`font-bold text-[20px] mb-3 ${isDarkTheme ? 'text-white' : 'text-gray-800'}`}>
                    {isEnglish ? (project.titleEn || project.title) : project.title}
                  </h3>
                  
                  <div className="flex gap-2 mb-3 flex-wrap">
                    <span className="text-[10px] px-2 py-1 rounded-md text-[#22c45e]" style={{ backgroundColor: 'rgba(34, 196, 94, 0.2)' }}>
                      {isEnglish ? (project.categoryEn || project.category) : project.category}
                    </span>
                    <span className={`text-[10px] px-2 py-1 rounded-md ${
                      isDarkTheme ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600'
                    }`}>
                      {getViewCount(project.id)} {isEnglish ? 'views' : '次浏览'}
                    </span>
                  </div>
                  
                  <p className={`text-[12px] flex-grow line-clamp-3 ${
