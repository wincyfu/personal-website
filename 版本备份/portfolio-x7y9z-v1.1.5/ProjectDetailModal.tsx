                  ...
                  <div className="relative aspect-[16/9] w-full">
                    <img
                      ...
                      onError={(e) => {
                        ...
                        // 判断是否是需要特殊处理的项目图片
                        const isSpecialProjectImage = image.includes('detail-2-') || 
                                                     image.includes('detail-3-') ||
                                                     project.title === '儿宝管家项目' ||
                                                     project.title === 'AI健康助手';
                        
                        if (isSpecialProjectImage) {
                          // 特殊项目图片特殊处理
                          console.log(`特殊项目图片加载失败，尝试修复: ${image}`);
                          ...
                          // 如果是detail-3-*.jpg或detail-3-*.png格式的图片
                          if (fileName.match(/detail-3-\d+\.(jpg|png)/)) {
                            const match = fileName.match(/detail-3-(\d+)\.(jpg|png)/);
                            const detailNum = match[1];
                            const ext = match[2];
                            // 添加更多可能的路径
                            tryPaths.push(
                              `/images/detail-3-${detailNum}.${ext}`,
                              `/images/portfolio/detail-3-${detailNum}.${ext}`,
                              `/images/portfolio/backup/detail-3-${detailNum}.${ext}`
                            );
                          }
                          ...
                          if (e.currentTarget) {
                            ...
                            setTimeout(() => {
                              if (e.currentTarget && e.currentTarget.naturalWidth === 0) {
                                tryNextPath();
                              }
                            }, 300);
                          }
